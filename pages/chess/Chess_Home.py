...
chess_data = pd.read_csv("./applicant_data/chess_applicant_data.csv")

tab1, tab2, tab3 = st.tabs(["🔎 Overview", "📈 Charts", "📍 Map"])
with tab1:
    # Basic Queries for IT Metrics
    average_age = round(chess_data['age'].mean(), 1)
    males = len(chess_data[chess_data.gender == "Male"])
    females = len(chess_data[chess_data.gender == "Female"])
    total_applicants = len(chess_data)
    university_students = len(chess_data[chess_data.occupation == "Student"])
    not_working = len(chess_data[chess_data.occupation == "not working"])
    doing_jobs = len(chess_data[chess_data.occupation == "Employed"])
    ngo_work = len(chess_data[chess_data.ngo_work == "yes"])

    st.header("Overview of Chess Applications")
    ...
    unemployed_metric.metric(label="Are not working", value=not_working)
...
with tab2:
    ...
    applicants_by_institute = chess_data["institute"].value_counts().reset_index()
    ...
    transport_counts = chess_data["transport"].value_counts().reset_index()
...
with tab3:
    tab3.subheader("Where are the Chess applicants coming from?")
    def get_coordinates(address):
        random_coords = (np.random.randn(2, 1) / [10, 10]) + [31.5204, 74.3587]
        return random_coords[0]

    map_df = chess_data.filter(items=["name", "transport", "city_address"])
