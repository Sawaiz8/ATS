...
def save_project(project_name, it_file, chess_file, sel_file):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if project_name in session_names:
        ...
        def download_resumes_and_update_paths(df, project_name, category):
            for name, resume in df[["name", "CV"]].values:
                file_path = f"./database/{project_name}/applicants_resume/{name.replace(' ', '_')}_resume_{category}.pdf"
                downloader.download_pdf(resume, file_path)
                df.loc[df.name == name, "path_to_pdf"] = file_path

        download_resumes_and_update_paths(it, project_name, "it")
        download_resumes_and_update_paths(sel, project_name, "sel") 
        download_resumes_and_update_paths(chess, project_name, "chess")

        it["applicant_status"] = "Under_Review"
        chess["applicant_status"] = "Under_Review"
        sel["applicant_status"] = "Under_Review"

        upsert_volunteers_data(df=it, project_name=project_name, category="it")
        upsert_volunteers_data(df=chess, project_name=project_name, category="chess") 
        upsert_volunteers_data(df=sel, project_name=project_name, category="sel")
        ...
        session_data_mongo["session_name"] = project_name
        ...
        for url, category in zip(sheet_urls, categories):
            ...
            session_data_mongo["categories"][category]["dataframe_path"] = f"./database/{project_name}/applicants_form_data/{category}_applicant_data.csv"
